...
const (
	...
	generatedSourcePrefixLimit    = 64 << 10
	generatedCgoBanner            = "// Code generated by cmd/cgo; DO NOT EDIT."
	generatedCgoDirective         = "//go:cgo_"
	generatedTestmainSourcePrefix = "// Code generated by 'go test'. DO NOT EDIT.\n\npackage main\n"
	indexedExportVersionLimit     = 16
)

func stripPackageArchiveToExport(path, exportPath string) (bool, error) {
	return stripPackageArchiveToExportWithFallback(path, exportPath, nil)
}

func stripPackageArchiveToExportWithFallback(path, exportPath string, onCopyFallback func()) (bool, error) {
	in, err := os.Open(path)
	if err != nil {
	...
	if err := replaceWithExportArchiveWithFallback(exportPath, path, onCopyFallback); err != nil {
		return false, err
	}
...
func retainEscapedGeneratedGoSourceWithFallback(path, retainedPath string, onCopyFallback func()) (retainedTypeKind, bool, error) {
	...
	kind, ok := retainedGeneratedSourceKind(prefix)
	if !ok {
		return 0, false, nil
	}
	if err := retainEscapedFileWithFallback(path, retainedPath, onCopyFallback); err != nil {
		return 0, false, err
	}
	return kind, true, nil
}

func retainEscapedIndexedExportDataWithFallback(path, retainedPath string, onCopyFallback func()) (bool, error) {
	prefix, err := readFilePrefix(path)
	if err != nil {
		return false, fmt.Errorf("read live file: %w", err)
	}
	if !isIndexedExportData(prefix) {
		return false, nil
	}
	if err := retainEscapedFileWithFallback(path, retainedPath, onCopyFallback); err != nil {
		return false, err
...
func retainEscapedFileWithFallback(path, retainedPath string, onCopyFallback func()) error {
	...
	if err := replaceWithExportArchiveWithFallback(retainedPath, path, onCopyFallback); err != nil {
		return err
	}
	return nil
}
...
func isGeneratedCgoSource(data []byte) bool {
	if !isLikelyText(data) || !bytes.Contains(data, []byte("package ")) {
		return false
	}
	// cmd/cgo writes its generated-code banner at the start of every generated
	// source file, so the bounded classification prefix is sufficient.
	return bytes.Contains(data, []byte(generatedCgoBanner)) ||
		bytes.Contains(data, []byte(generatedCgoDirective))
}

func isGeneratedTestmainSource(data []byte) bool {
	data = bytes.TrimLeft(data, " \t\r\n")
	return bytes.HasPrefix(data, []byte(generatedTestmainSourcePrefix))
}

func isIndexedExportData(data []byte) bool {
	if len(data) < 1+3 || data[0] != 'i' {
		return false
	}
	prefixComplete := len(data) < generatedSourcePrefixLimit
	data = data[1:]
	version, n := binary.Uvarint(data)
	if n <= 0 || version > indexedExportVersionLimit {
		return false
	}
	data = data[n:]
	stringSize, n := binary.Uvarint(data)
	if n <= 0 {
		return false
	}
	data = data[n:]
	declSize, n := binary.Uvarint(data)
	if n <= 0 {
		return false
	}
	data = data[n:]

	// Strings and declarations are followed by the package index. For a
	// bounded prefix, accepting a payload that extends beyond the prefix is
	// safe after the header and sizes have been validated.
	payloadSize := stringSize + declSize
	if payloadSize < stringSize {
		return false
	}
	return payloadSize < uint64(len(data)) || !prefixComplete
}
...
func readFilePrefix(path string) ([]byte, error) {
	...
	return io.ReadAll(io.LimitReader(file, generatedSourcePrefixLimit))
}
