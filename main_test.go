...
func TestCloseRetainsGeneratedCgoSourceLiveFile(t *testing.T) {
	...
	actionID := bytes.Repeat([]byte{59}, 32)
	outputID := bytes.Repeat([]byte{60}, 32)
	res, err := st.put(request{
		ID:       1,
		Command:  cmdPut,
		ActionID: actionID,
...
func TestRunStatusEmptyCache(t *testing.T) {
	...
	assertContains(t, got, "20.0GiB (21474836480 bytes)")
	...
	assertContains(t, got, "Uncompressed size  0B (0 bytes)")
	assertContains(t, got, "Compressed size    0B (0 bytes)")
	assertContains(t, got, "Savings            0B (0.0%)")
	assertContains(t, got, "Storage:\n")
	assertContains(t, got, "Blobs           0      0B (0 bytes)")
	assertContains(t, got, "Retained files  0      0B (0 bytes)")
	assertContains(t, got, "Blob types (best effort):\n")
	assertContains(t, got, "None  0      0B (0 bytes)  0B (0 bytes)")
...
func TestRunStatusCountsUnreadableBlobTypes(t *testing.T) {
	...
	got := stdout.String()
	assertContains(t, got, "Blob types (best effort):")
	assertContains(t, got, "Unreadable blobs  1      4B (4 bytes)")
}
